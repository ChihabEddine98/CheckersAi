...
public class ComputerPlayer extends Player {
	...
	protected void getMoveWeight(Game game, Move m) {
		...
		m.changeWeight(getSafetyWeight(b, game.isP2Turn()));
		...
		if (changed) {
			...
			int depth = getSkipDepth(game, endIndex, !game.isP2Turn());
			if (safeAfter) {
		...
		// Check how many more skips are available
			...
			int depth = getSkipDepth(game, startIndex, game.isP2Turn());
		...
		m.changeWeight(getSafetyWeight(b,
				changed? !game.isP2Turn() : game.isP2Turn()));
	}
