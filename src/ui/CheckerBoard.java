...
public class CheckerBoard extends JButton {
	...
	public void paint(Graphics g) {
		...
		String msg = game.isP2Turn()? "Player 2's turn" : "Player 1's turn";
		int width = g.getFontMetrics().stringWidth(msg);
		Color back = game.isP2Turn()? Color.BLACK : Color.WHITE;
		Color front = game.isP2Turn()? Color.WHITE : Color.BLACK;
		g.setColor(back);
	...
	public void setPlayer1(Player player1) {
		...
		if (game.isP2Turn() && !this.player1.isHuman()) {
			this.selected = null;
		}
	}
	...
	public void setPlayer2(Player player2) {
		...
		if (!game.isP2Turn() && !this.player2.isHuman()) {
			this.selected = null;
		}
	}
	
	public Player getCurrentPlayer() {
		return game.isP2Turn()? player1 : player2;
	}
	...
	private void handleClick(int x, int y) {
		...
		this.selectionValid = isValidSelection(
				copy.getBoard(), copy.isP2Turn(), selected);
